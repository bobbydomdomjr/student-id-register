...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Check if gender is set to prevent undefined key warning
    ...
    $gender = isset($_POST['gender']) ? $_POST['gender'] : ''; // Avoid undefined error
    ...
    if (empty($studentno) || empty($lastname) || empty($firstname) || empty($dob) || empty($email) || empty($gender) || empty($phone) || empty($course) || empty($yearlevel) || empty($address) || empty($contactname) || empty($contactno) || empty($relationship)) {
        die("<script>alert('Please fill out all required fields!'); window.location.href='new_student.php';</script>");
    }

    // Prepare SQL statement
    $sql = "INSERT INTO student_registration (studentno, lastname, firstname, middleinitial, dob, email, gender, phone, course, yearlevel, block, address, contactname, contactno, relationship)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
